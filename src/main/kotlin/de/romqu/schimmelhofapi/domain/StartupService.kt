...
class StartupService(
    ...
    companion object {
        ...
        const val WEEK_MILLIS = 86400000L * 7
    }
    ...
    fun execute() {
        ...
        Timer().scheduleAtFixedRate(time = nextMondayAtMidnightDate, period = WEEK_MILLIS) {
            dayRepository.getAll().lastOrNull()?.
        }
    }

    fun getDaysOfThreeWeeks(): List<LocalDate> =
        datesOfWeekDays(LocalDate.now(), 3)

    fun datesOfWeekDays(date: LocalDate, nextWeeks: Int): List<LocalDate> {

        val firstMonday = date.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY))

        val plusDays = (nextWeeks * DayOfWeek.values().size) - 1L
